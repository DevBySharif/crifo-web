...
let releasePromise: Promise<Record<string, unknown> | null> | null = null;

function fetchRelease(): Promise<Record<string, unknown> | null> {
  if (!releasePromise) {
...
export function DownloadButton({ variant = "hero" }: { variant?: "hero" | "cta" }) {
  ...
  return (
    ...
    >
      <DownloadIcon className="h-5 w-5 transition group-hover:-translate-y-0.5" />
      Download APK
    </a>
  );
}
